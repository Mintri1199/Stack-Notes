...
class TodoStore: NSObject {
  ...
  // A Better fetch data
  func modifiedFetch(completion: (Result<[TodoPersistent], Error>) -> Void) {
    // Fetch the small todos and the other todo in order to order them
    let smallTodoFetchRequest: NSFetchRequest<TodoPersistent> = TodoPersistent.fetchRequest()
    let fetchRequest: NSFetchRequest<TodoPersistent> = TodoPersistent.fetchRequest()
    smallTodoFetchRequest.predicate = NSPredicate(format: "small == %@", NSNumber(value: true))
    fetchRequest.predicate = NSPredicate(format: "small == %@", NSNumber(value: false))
    let viewContext = container.viewContext
    
    do {
      let smallTodos = try viewContext.fetch(smallTodoFetchRequest)
      let theRest = try viewContext.fetch(fetchRequest)
      completion(.success(smallTodos + theRest))
    } catch {
      completion(.failure(error))
    }
  }
